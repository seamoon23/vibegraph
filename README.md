...
> (스크린샷 추가 방법은 [`docs/screenshots/README.md`](docs/screenshots/README.md) 참고)
...
핵심은 **"같은 Claude 창"에서 채점**한다는 점입니다. 그 창은 이미 대화 전체 맥락을 갖고 있으니,
대화를 따로 복사해 붙일 필요 없이 프롬프트 한 줄이면 정확한 채점이 나옵니다. CLI는 폴더 정리·HTML 생성·통계만 담당합니다.
...
1. 저장소를 클론(또는 ZIP 다운로드)해 원하는 위치에 둡니다. (한글·공백 없는 경로 권장)
   ```bash
